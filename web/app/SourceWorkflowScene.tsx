...
export function useSourceWorkflowScene({ graphNode, focusGroupId, operators, promotedInvocationIds, onFocusChange, onViewChange, onReplace, onPromote, onReset }: {
  ...
  const [nodes, setNodes, applyNodeChanges] = useNodesState<SourceFlowNode>([]);
  const [edges, setEdges, applyEdgeChanges] = useEdgesState<SourceFlowEdge>([]);
  ...
  const sceneEdges = useMemo<SourceFlowEdge[]>(() => projection?.relations.map((relation) => {
    ...
    return { id: relation.id, source: relation.from, target: relation.to, type: "smoothstep", className: `source-outline-edge relation-${relation.kind} ${relation.proxied ? "is-proxied" : ""} ${boundary ? "is-boundary" : ""}`, selectable: !boundary, deletable: false, focusable: true, ariaLabel: `${relation.member_count} source relation${relation.member_count === 1 ? "" : "s"}; not a typed data wire`, domAttributes: { "data-testid": boundary ? `source-boundary-relation-${relation.id}` : relation.proxied ? `source-proxy-relation-${relation.id}` : `source-relation-${relation.id}` } as unknown as NonNullable<SourceFlowEdge["domAttributes"]>, data: { relation } };
