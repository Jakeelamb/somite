...
use axial_cook::{cook_graph, NodeState, Project};
use axial_ir::Graph;
use axial_ops::Catalog;
use axial_paper::{extract_from_path, reconstruct};
...
fn main() -> Result<()> {
    ...
    match cmd.as_str() {
        "cook" => {
            let graph_path = args.next().context("axial cook <graph.axial.json>")?;
            cook_cmd(Path::new(&graph_path))?;
        }
        "paper" => {
            let paper_path = args.next().context("axial paper <methods.txt|paper.pdf>")?;
