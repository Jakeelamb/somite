...
//! already standalone Axial executables.

use std::collections::{BTreeMap, BTreeSet, VecDeque};

use axial_ir::{Direction, Edge, Graph, Layout, Node, ParamValue, Port, PortType, SCHEMA_VERSION};
...
mod tests {
    ...
    fn nfcore_read_entry_process_accepts_separate_fastq_mates_only_at_the_boundary() {
        ...
        assert!(axial_ir::compatible(PortType::Fastq, r1.ty, &r1.union));
        assert!(axial_ir::compatible(PortType::FastqGz, r1.ty, &r1.union));
        assert!(r2.optional);
        ...
        assert!(!axial_ir::compatible(PortType::Fastq, input.ty, &input.union));
    }
