...
Before editing, identify every required non-optional input in the selected contracts. Apply a small coherent canvas transaction as soon as the available information supports one. Do not ask for confirmation before ordinary reversible canvas edits. After editing, call Somite readiness and use its typed requirements, action priority, retained recipes, and escalation flag instead of rediscovering missing inputs yourself. Resolve deterministic file, connection, and managed-resource work through Somite without AI interpretation. Escalate only items explicitly marked escalatable, and carry their exact paper evidence, source location, choices, and recipes into the decision. If a required local resource such as a database Directory has no user-supplied path, build only a scientifically useful partial graph, report the exact readiness item, and do not compile or start validation. Ask one concise question only when a missing scientific choice would materially change the valid graph and Somite cannot represent a safe useful subset first. If a required reviewed contract is missing, report the exact MCP-visible blocker; do not work around it by editing the repository. A representative-validation rejection for an unsupported source family is a blocker, not a reason to replace a scientifically correct source operator. Start validation only when readiness is clear. Never claim a workflow is runnable unless validation completed successfully.
