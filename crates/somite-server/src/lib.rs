...
use somite_bundle::{build_bundle, BundlePlan, ExportTarget};
use somite_cook::{cook_graph, pixi_available, ArtifactMeta, NodeState, Project};
use somite_ir::Graph;
use somite_ops::{current_pixi_platform, Catalog, Operator};
use somite_ops::{nfcore, snakemake, workflow};
use somite_paper::{
...
impl WebProject {
    ...
    pub fn session(&self) -> Result<ProjectSession, ServerError> {
        let recovery_path = self.root.join(".somite/autosave.somite.json");
    ...
    pub fn save_autosave(&self, graph: &Graph) -> Result<(), ServerError> {
        self.save_graph_at(&self.root.join(".somite/autosave.somite.json"), graph)
    }
...
async fn discover_snakemake(
    ...
    let cache_path = project.root.join(".somite/catalog/snakemake-workflows.json");
    let response = tokio::task::spawn_blocking(move || {
...
fn project_bundle(
    project: &WebProject,
    graph: &Graph,
) -> Result<somite_bundle::ExportBundle, ServerError> {
    let mut catalog = project.catalog.clone();
...
mod tests {
    ...
    async fn session_recovers_a_valid_autosave() {
        ...
        std::fs::create_dir(temp.path().join(".somite")).expect("recovery directory");
        std::fs::write(
    ...
    async fn export_endpoints_return_a_plan_and_downloadable_zip() {
        ...
        let disposition = zip_response
            ...
            .unwrap_or_default();
        assert!(disposition.ends_with(".somite.zip\""));
    ...
    async fn autosave_validates_and_writes_a_recovery_graph() {
        ...
        let response = app(project)
            ...
            .expect("response");
        assert_eq!(response.status(), StatusCode::OK);
        let recovery = std::fs::read_to_string(temp.path().join(".somite/autosave.somite.json"))
            .expect("recovery graph");
